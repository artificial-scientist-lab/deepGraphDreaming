...
from graph_dreamer import generatorGraphFidelity, dream_model
...
def train_model(NN_INPUT_SIZE, NN_OUTPUT_SIZE, vals_train_np, res_train_np, vals_test_np, res_test_np, learnRate, save_direc, suffix, num_of_examples, nnType, l2Lambda = 0.001, l2Regularization = False):
    ...
    batch_size=min(len(vals_train_np),5e3) # Large batch_size seems to be important (smaller number of examples the greater the number of batches)
    ...
    for episode in range(100000): # should be much larger, with good early stopping criteria
        ...
        with torch.no_grad():
            ...
            if len(test_loss_evolution)-np.argmin(test_loss_evolution)>500:
                print('    Early stopping kicked in: test loss',np.argmin(test_loss_evolution),len(test_loss_evolution))
                break  
